...
function displayXeroSetup_() {
  ...
  panel1.add(grid).setStyleAttributes(SUB_PANEL_CSS);    
  app.add(label1);
  app.add(panel1);  
  ss.show(app);  
}
...
function saveSettings_(e) {
  ...
  if (p.appType != 'Private') {
    ...
    if (result !== '') {
      ...
      var link = app.createAnchor('Connect to Xero', true, result);
      ...
      Log.warning('Failed to get auth URL')
    }
...
// accountsDownload_() moved to Accounts_.gs
...
function downloadOptions(item) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();    
  var app = UiApp.createApplication();
  var panel = app.createAbsolutePanel().setStyleAttributes(SUB_PANEL_CSS).setWidth("475");  
...
function uploadInvoices(){
  ...
  } // uploadInvoices.logFailedInvoices()
...
function uploadPayments() {
  ...
  } // uploadPayments.logFailedPayments()
